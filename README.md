...
| 方式 | 说明 |
| --- | --- |
| 齿轮面板 | 鱼数量 1–16，画质、帧率；**天色**（自动 / 昼 / 黄昏 / 夜）和**天气**（自动 / 晴 / 阴 / 雨 / 雾）当场预览 |
| 查询参数 | `index.html?fish=8&quality=low&fps=24&lat=31.23&lon=121.47&ui=0` |
| 天色 / 天气预览 | 右下角齿轮即可，不必改地址栏。也可 `?time=night&weather=rain`（`day` / `dusk`，`clear` / `cloudy` / `fog`；`?hour=21`）。选「自动」或「恢复自动」则跟真实时钟和地点 |
| Windows 托盘 | 壁纸模式下也可在托盘选天色 / 天气预览 |
| Lively 自定义 | 同上，且按显示器分别记住；可填纬度和经度 |
| `?demo=1` | 每隔几秒自动投喂，方便录预览 |

画质大致对应：

- **low**：更低涟漪分辨率、无焦散、像素密度 1x；雨天几乎不画雨丝；鱼身切片较少
- **mid**：默认；软焦散、中等涟漪；可见雨丝；鱼被水面轻轻折射
- **high**：更密涟漪、更高 DPR，4K 上更清晰也更耗电；雨丝更多，折射仍保持克制
...
js/water.js         WebGL2 水面、高度场涟漪，并把锦鲤折射进水里（失败则 Canvas 2D）
js/sprites.js       俯视程序化锦鲤与荷叶（画布路径，不是直播 SVG）
js/world.js         闲游 / 趋食 / IK 脊柱；鱼身用平滑丝带而不是方块切片
...
趋食仍是点击投喂：最多 3 条鱼去吃，靠近时 arrive 减速，并略微放松转弯半径，以免鱼食落在不可达的转弯圆里。画质档只改脊柱采样密度（约 16 / 24 / 32），不改这套力学。鱼从俯视绘制，轮廓是一条沿脊柱的平滑丝带，避免侧视方块切片的积木感。
...
- **白天**：暖青水色、强焦散，鱼偏暖。
- **黄昏**：琥珀色天光（`?time=dusk` 可强制）。
- **夜晚**：靛蓝水面、月亮和冷光，焦散几乎关掉，鱼偏青银。
- **天气**：在线时向 [Open-Meteo](https://open-meteo.com/) 拉当前天气（不用 API key），映射为晴 / 阴 / 雨 / 雾。雨天有明显雨丝和点滴涟漪；阴天压亮、压焦散；雾天一层厚白纱。风力会轻轻推涟漪和环境流。
- **预览（不写进天气缓存）**：齿轮「天色 / 天气」或地址栏 `?time=night&weather=rain`。默认仍是真实时钟 + 地点（已授权定位 → IP → 上海）。`?hour=21` 按太阳时近似夜间。
...
没有网络时池塘仍然是完整壁纸，只是不会更新实时云雨。涟漪会折射鱼和鱼食，不只装饰空水面。
